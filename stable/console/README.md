...
SUSE Stratos Console is an Open Source Web-based Management console for Cloud Foundry and Kubernetes. It allows users and administrators to both manage applications running in the Cloud Foundry cluster and perform cluster management tasks.

## Installation

SUSE Stratos Console can be installed to a Kubernetes cluster using Helm. Either Helm 2 or Helm 3 can be used, although we recommend using the newer Helm 3 version.
...
The Helm chart is published to the SUSE Helm repository.
...
suse/console      	4.3.0           2.1.1      	A Helm chart for deploying SUSE Stratos Console
```
...
To install SUSE Stratos Console:

```
kubectl create namespace console
helm install my-console suse/console   --namespace=console
```

> **Note**: The first `kubectl` command will create a namespace for SUSE Stratos Console. With Helm 3 you must create a namespace before installing.
We recommend installing SUSE Stratos Console into a separate namespace.
...
This will create a SUSE Stratos Console instance named `my-console` in a namespace called `console` in your Kubernetes cluster.
...
The following table lists the configurable parameters of the SUSE Stratos Console Helm chart and their default values.
...
|console.apiKeysEnabled|Enable/disable API key-based access to SUSE Stratos Console API (disabled, admin_only, all_users)|admin_only|
...
|uaa.consoleAdminIdentifier|Scope that identifies an admin user of SUSE Stratos Console (e.g. cloud_controller.admin||
...
> In this example, SUSE Stratos Console has been deployed withe the service configured as `NodePort`.

The URL you use for accessing SUSE Stratos Console will depend on the service configuration and the environment that you have used.

> Note: If you did not provide a certificate when installing, SUSE Stratos Console will use a self-signed certificate, so you will see a certificate warning which you access SUSE Stratos Console in a browser.
...
To uninstall SUSE Stratos Console, delete the Helm release and also delete the Kubernetes namespace:
...
> Note: SUSE Stratos Console creates secrets in the namespace as part of an initialization job. These are not managed by Helm, so make sure you
...
```
kubectl create namespace console
helm install my-console suse/console   --namespace=console --set console.service.type=LoadBalancer
```
...
```
kubectl create namespace console
helm install my-console suse/console   --namespace=console -f private_overrides.yaml
```

## Deploying with your own TLS certificates

By default the console will generate self-signed certificates for demo purposes. To configure SUSE Stratos Console to use your provided TLS certificate, create a TLS secret in the namespace you are installing into and specify this secret name using the helm chart value `console.tlsSecretName` when installing.
...
> Where `NAMESPACE` is the namespace you are installing SUSE Stratos Console into - you will need to manually create the namespace first if it does not already exist.

You can now install SUSE Stratos Console with:

```
kubectl create namespace console
helm install my-console suse/console   --namespace=console --set console.tlsSecretName=stratos-tls-secret

```

## Using an External Database

You can choose to use SUSE Stratos Console with an external database, rather than deploying a single-node MariaDB instance as part of the Helm install.
...
When using an external database server, SUSE Stratos Console expects that you have:

- Created a user that will be used to access the database
- Created a database for the SUSE Stratos Console tables and data
...
When deploying with SCF, the `scf-config-values.yaml` (see [SCF Wiki link](https://github.com/SUSE/scf/wiki/How-to-Install-SCF#configuring-the-deployment)) can be supplied when installing SUSE Stratos Console.

```
kubectl create namespace console
helm install my-console suse/console   --namespace=console -f scf-config-values.yaml
```
...
To install SUSE Stratos Console with the above specified configuration:

```
kubectl create namespace console
helm install my-console suse/console   --namespace=console -f uaa-config.yaml
```
...
```
kubectl create namespace console
helm install my-console suse/console   --namespace=console --set console.localAdminPassword=<password>
```
...
SUSE Stratos Console uses persistent volumes. In order to deploy it in your Kubernetes environment, you must
have a storage class available.

Without configuration, the SUSE Stratos Console Helm Chart will use the default storage class. If a default storage
...
If no default storage class has been defined in the Kubernetes cluster. The SUSE Stratos Console helm chart will fail to deploy successfully. To check if a `default` storage class exists, you can list your configured storage classes with `kubectl`. If no storage class has `(default)` after it, then you need to either specify a storage class override or declare a default storage class for your Kubernetes cluster.
...
```
kubectl create namespace console
helm install my-console suse/console   --namespace=console -f override.yaml
```
